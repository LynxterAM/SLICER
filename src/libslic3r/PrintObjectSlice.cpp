...
void PrintObject::_max_overhang_threshold() {
    ...
    const coord_t resolution = std::max(scale_t(m_print->config().resolution / 2), SCALED_EPSILON);
                        ...
                        for (size_t other_layer_bridge_idx = layer_idx + 1; other_layer_bridge_idx < max_layer_idx; other_layer_bridge_idx++) {
                            ...
                            for (size_t other_region_idx = 0; other_region_idx < my_layer->m_regions.size(); ++other_region_idx) {
                                ...
                                if ( (other_lregion->region().config().overhangs_bridge_threshold.value != 0 ||
                                    ...
                                    const Surfaces &other_surfaces = other_lregion->m_slices.surfaces;
                                    for (const ExPolygon &other_to_bridge : intersection_ex(still_unsupported, to_expolygons(other_surfaces))) {
...
std::vector<ExPolygons> PrintObject::slice_support_column_tops() const
{
    ModelVolumePtrs::const_iterator it_volume = this->model_object()->volumes.begin();
    ModelVolumePtrs::const_iterator it_volume_end = this->model_object()->volumes.end();
    for (; it_volume != it_volume_end && !(*it_volume)->is_support_column(); ++it_volume) ;
    if (it_volume == it_volume_end)
        return {};

    // Filled support columns use only the highest non-empty slice of each
    // column volume.  Lower slices are ignored so a tall column contributes one
    // artificial top seed, not one seed per object layer it intersects.
    std::vector<float> zs = zs_from_layers(this->layers());
    std::vector<ExPolygons> slices(zs.size());
    std::vector<char> merge_layers(zs.size(), false);
    bool merge = false;
    const Print *print = this->print();
    std::function<void()> throw_on_cancel_callback = [print]() { print->throw_if_canceled(); };
    MeshSlicingParamsEx params;
    params.trafo = this->trafo_centered();

    for (; it_volume != it_volume_end; ++it_volume) {
        if (!(*it_volume)->is_support_column())
            continue;

        std::vector<ExPolygons> volume_slices = slice_volume(*(*it_volume), zs, params, throw_on_cancel_callback);
        for (size_t layer_id = volume_slices.size(); layer_id > 0; --layer_id) {
            const size_t top_layer_id = layer_id - 1;
            if (volume_slices[top_layer_id].empty())
                continue;

            if (slices[top_layer_id].empty()) {
                slices[top_layer_id] = std::move(volume_slices[top_layer_id]);
            } else {
                append(slices[top_layer_id], std::move(volume_slices[top_layer_id]));
                merge_layers[top_layer_id] = true;
                merge = true;
            }
            break;
        }
    ...
    if (merge) {
        ...
        for (size_t layer_id = 0; layer_id < zs.size(); ++layer_id)
            if (merge_layers[layer_id])
                to_merge.emplace_back(&slices[layer_id]);
        Slic3r::parallel_for(size_t(0), to_merge.size(),
            [&to_merge](const size_t i) {
                *to_merge[i] = union_ex(*to_merge[i]);
            });

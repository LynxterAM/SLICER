...
const ColorRGBA GLVolume::SUPPORT_COLUMN_COLOR     = {0.2f, 0.8f, 0.2f, 0.5f}; // green
    ...
    if (force_native_color || force_neutral_color) {
        ...
        if (hover == HS_Deselect)
            ...
            if (color == NEGATIVE_VOLUME_COLOR || color == PARAMETER_MODIFIER_COLOR ||
                ...
                color == SUPPORT_COLUMN_COLOR || color == SEAM_POSITION_COLOR ||
                color == BRIM_PATCH_COLOR || color == BRIM_NEGATIVE_COLOR)
...
ColorRGBA color_from_model_volume(const ModelVolume& model_volume)
{
    ...
    else if (model_volume.is_support_column())
        color = GLVolume::SUPPORT_COLUMN_COLOR;
